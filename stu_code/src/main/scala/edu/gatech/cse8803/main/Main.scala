...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val deadPatient = patient.filter(x => x.expireFlag == 1).keyBy(x => x.patientID).join(features.keyBy(f => f._1)).map(x => LabeledPoint(1.0, x._2._2._2))
    val alivePatient = patient.filter(x => x.expireFlag == 0).keyBy(x => x.patientID).join(features.keyBy(f => f._1)).map(x => LabeledPoint(0.0, x._2._2._2))
  ...
  def loadRddRawData(sqlContext: SQLContext): (RDD[LabEvent], RDD[Prescription], RDD[Procedures], RDD[Patient]) = {
    ...
    val labEventsDF =  sqlContext.sql("SELECT P.SUBJECT_ID, L.ITEMID, 0 AS VALUENUM FROM PATIENTS P INNER JOIN ADMISSIONS A ON P.SUBJECT_ID = A.SUBJECT_ID LEFT JOIN DIAGNOSES_ICD D ON D.HADM_ID = A.HADM_ID INNER JOIN LABEVENTS L ON L.HADM_ID = A.HADM_ID WHERE D.ICD9_CODE IN ('78552','99591','99592') AND VALUENUM IS NOT NULL LIMIT 100 ".stripMargin).map(r => LabEvent(r(0).toString, r(1).toString, r(2).toString.toDouble))
    labEventsDF.cache()
    println(labEventsDF.count())


    // TRANSFORM: VALUENUM to new field Feature mapped to VALUE based on classification
   ...
   // TRANSFORM: DOSE_VAL_RX to new field Feature mapped to VALUE based on classification
    val prescription: RDD[Prescription] =  sqlContext.sql("SELECT P.SUBJECT_ID, PR.DRUG, DOSE_VAL_RX, 0 as VALUE FROM PATIENTS P INNER JOIN ADMISSIONS A ON P.SUBJECT_ID = A.SUBJECT_ID LEFT JOIN DIAGNOSES_ICD D ON D.HADM_ID = A.HADM_ID INNER JOIN PRESCRIPTIONS PR ON PR.HADM_ID = A.HADM_ID WHERE D.ICD9_CODE IN ('78552','99591','99592') LIMIT 100 ".stripMargin).map(r => Prescription(r(0).toString, r(1).toString, r(2).toString, r(3).toString.toDouble))
    ...
    //Add GENDERINDICATOR, DOB, DOB to Patients
